...
import { generateRound, seeded, type Match, type Player } from "./pairing.ts";
...
test("9 players / 2 courts: 8 play, 1 benched, nobody twice", () => {
  const r = generateRound(mk(9), 2, [], seeded(1));
...
test("6 players / 2 courts: one doubles, one singles", () => {
  const r = generateRound(mk(6), 2, [], seeded(1));
...
test("balanced teams when rated", () => {
  ...
  const r = generateRound(players, 1, [], seeded(1));
...
test("unrated players still get a round", () => {
  const r = generateRound(mk(8, false), 2, [], seeded(1));
  assert.equal(r.matches.length, 2);
});

test("seeded rng makes generateRound reproducible", () => {
  const a = generateRound(mk(9), 2, [], seeded(42));
  const b = generateRound(mk(9), 2, [], seeded(42));
  const c = generateRound(mk(9), 2, [], seeded(43));
